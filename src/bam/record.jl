...
"""
    isunmapped(record::Record)::Bool

Test if `record` is ummapped.
"""
function isunmapped(record::Record)::Bool
	return !(flag(record) & SAM.FLAG_UNMAP == 0)
end
...
"""
function isduplicated(record::Record)::Bool
    return flag(record) & 0x400 == 0
end
...
"""
	inferquerylength(record::Record)::Int

Get the alignment length of `record`. sum of cigar string except Hardclipping.
Same with pysam's infer_query_length()
"""
function inferquerylength(record::Record)::Int
    offset = seqname_length(record)
    length::Int = 0
    for i in offset + 1:4:offset + n_cigar_op(record, false) * 4
        x = unsafe_load(Ptr{UInt32}(pointer(record.data, i)))
